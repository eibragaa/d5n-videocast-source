...
REPO = Path(os.environ.get("D5N_BASE", Path(__file__).resolve().parent))
...
def main() -> None:
    ...
    for ep in reversed(counter["history"]):  # newest first
        ...
        guid = f"{BASE_URL}/?ep={ep['num']}"

        episodes.append(f"""    <item>
      <title>D5N • Episódio #{ep['num']} — {ep['date']}</title>
      <itunes:title>D5N • Episódio #{ep['num']}</itunes:title>
      ...
      <description>Drop Five News — edição de {ep['date']}. Curadoria diária de notícias em áudio.</description>
      <itunes:summary>Drop Five News — edição de {ep['date']}. Curadoria diária de notícias em áudio.</itunes:summary>
    </item>""")
...
<rss version="2.0" 
  ...
  <channel>
    ...
    <description>Curadoria diária de notícias em áudio. O D5N entrega todos os dias um resumo sonoro com as notícias mais relevantes do Brasil e do mundo — direto, sem enrolação.</description>
    <itunes:subtitle>Curadoria diária de notícias em áudio</itunes:subtitle>
    <itunes:summary>Curadoria diária de notícias em áudio. O D5N entrega todos os dias um resumo sonoro com as notícias mais relevantes do Brasil e do mundo — direto, sem enrolação.</itunes:summary>
    ...
    <itunes:keywords>notícias,brasil,mundo,política,economia,tecnologia,podcast,diário</itunes:keywords>
    <itunes:type>episodic</itunes:type>
    <itunes:complete>No</itunes:complete>
    <itunes:new-feed-url>{BASE_URL}/podcast.xml</itunes:new-feed-url>
    {ITUNES_CATEGORIES}
    ...
    <copyright>© {datetime.now().year} Drop Five News</copyright>
    <webMaster>eibragaa@gmail.com (Jean Braga)</webMaster>
    <generator>D5N Pipeline v1</generator>
{items_xml}
  </channel>
</rss>"""
