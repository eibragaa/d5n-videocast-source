...
if [[ ! "$AUDIO_NAME" =~ ^fechamento-([0-9]{4}-[0-9]{2}-[0-9]{2})\.mp3$ ]]; then
  printf 'ERRO: áudio não canônico: %s\n' "$AUDIO_NAME" >&2
  exit 2
fi
RELEASE_DATE="${BASH_REMATCH[1]}"
[ "$(basename "$SOURCE")" = "source-fechamento-$RELEASE_DATE.md" ] || { printf 'ERRO: source não corresponde à data do áudio.\n' >&2; exit 2; }
...
python3 "$FC_ROOT/scripts/gerar_fechamento_feed.py" --repo "$FC_ROOT"
python3 "$FC_ROOT/scripts/gerar_fechamento_feed.py" --repo "$FC_ROOT" --check-date "$RELEASE_DATE"
...
if git diff --cached --quiet -- "$AUDIO" "$SOURCE" "$MANIFEST" index.html "$FEED"; then
  printf 'Site da Fechamento do Mercado já está publicado para %s.\n' "$RELEASE_DATE"
  exit 0
fi

git commit --only -m "feat: publicar Fechamento do Mercado de $RELEASE_DATE" -- \
...
if git merge-base --is-ancestor origin/master HEAD; then
  ...
  SAFETY_BRANCH="fechamento/pre-push-$RELEASE_DATE"
  ...
  if ! git commit --only -m "feat: publicar Fechamento do Mercado de $RELEASE_DATE" -- \
...
printf 'Fechamento do Mercado publicada no site: %s\n' "$RELEASE_DATE"
