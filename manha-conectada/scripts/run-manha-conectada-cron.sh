...
LOG_DIR="$MC_ROOT/reports/cron"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(TZ=America/Sao_Paulo date '+%Y-%m-%d_%H-%M-%S').log"
TMP_RESULT=$(mktemp)
trap 'rm -f "$TMP_RESULT"' EXIT

if ! python3 "$MC_ROOT/scripts/manha_conectada_pipeline.py" >"$TMP_RESULT" 2>"$LOG_FILE"; then
...
if ! bash "$MC_ROOT/scripts/publish-manha-conectada-site.sh" "$FILE" "$SOURCE" "$MANIFEST" >>"$LOG_FILE" 2>&1; then
